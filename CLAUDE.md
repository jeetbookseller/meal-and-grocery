...
15. **Explicit Data API grants**: Supabase no longer grants Data API roles access to new tables in `public` by default (new projects from 2026-05-30, all existing projects from 2026-10-30). Migration `007_data_api_grants.sql` grants access on all existing tables and client-callable RPCs. **Every future migration that creates a table in `public` MUST include** `GRANT SELECT, INSERT, UPDATE, DELETE ON public.<table> TO authenticated, service_role;` plus `ENABLE ROW LEVEL SECURITY` and policies. New client-callable RPC functions need `GRANT EXECUTE ... TO authenticated, service_role;` followed by `REVOKE EXECUTE ... FROM PUBLIC, anon;` (Postgres grants EXECUTE to PUBLIC by default, exposing SECURITY DEFINER functions to anonymous RPC calls); internal-only functions are also revoked from `authenticated`. `anon` is deliberately never granted — every RLS policy requires `auth.uid()`, so anonymous requests can never see rows.
...
| `supabase/migrations/007_data_api_grants.sql` | Explicit Data API grants for all tables + RPCs (template for future migrations) |
