<template>
  ...
  <div v-else-if="authStore.resetPasswordSent" class="space-y-4 text-center">
    <div class="p-3 rounded-md bg-accent/10 border border-accent/30 text-accent text-sm">
      Password reset email sent! Please check your inbox.
    </div>
    <p class="text-sm text-text-secondary">
      <button type="button" @click="backToLogin" class="text-accent hover:underline font-medium">Back to sign in</button>
    </p>
  </div>
  ...
  <form v-else @submit.prevent="handleSubmit" class="space-y-4">
    ...
    <template v-if="mode !== 'forgot'">
      <div>
        ...
        <div v-if="mode === 'login'" class="mt-1 text-right">
          <button type="button" @click="enterForgotMode" class="text-xs text-text-secondary hover:text-accent hover:underline">Forgot password?</button>
        </div>
      </div>
