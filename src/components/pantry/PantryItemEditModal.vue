...
async function handleSave() {
  ...
  error.value = null
  try {
    await pantryStore.updateItem(props.item.id, {
      name: name.value.trim(),
      quantity: quantity.value || null,
    })
    await pantryStore.linkItemToMeals(props.item.id, selectedMealIds.value)
    emit('close')
  } catch (e) {
...
async function handleDelete() {
  ...
  error.value = null
  try {
    await pantryStore.deleteItem(props.item.id)
    emit('close')
  } catch (e) {
